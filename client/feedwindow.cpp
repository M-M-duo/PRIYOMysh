...
static void showCustomWarning(QWidget *parent, const QString &text) {
  ...
  QPixmap scaled =
      original.scaled(64, 64, Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
private:
  void updateImage() {
    ...
    if (!pixmap.isNull()) {
      QPixmap scaled = pixmap.scaled(400, 400, Qt::KeepAspectRatio,
...
void FeedWindow::setupUI() {
  ...
  if (currentUsername.isEmpty()) {
    ...
    connect(createPostButton, &QPushButton::clicked, this,
            &FeedWindow::onCreatePost);
    connect(findFriendsButton, &QPushButton::clicked, this,
            &FeedWindow::onFindFriendsClicked);
...
void FeedWindow::loadProfileInfo() {
  ...
  QNetworkReply *reply = networkManager->get(request);
  connect(reply, &QNetworkReply::finished,
          [this, reply]() { onProfileInfoFinished(reply); });
}
...
void FeedWindow::loadPosts(bool append) {
  ...
  if (currentUsername == "me") {
    endpoint =
        QString("http://127.0.0.1:8080/api/posts/feed/my?limit=%1&offset=%2")
            .arg(limit)
            .arg(currentOffset);
  } else if (!currentUsername.isEmpty() && currentUsername != "me") {
    endpoint =
        QString("http://127.0.0.1:8080/api/posts/feed/%1?limit=%2&offset=%3")
            .arg(currentUsername)
            .arg(limit)
            .arg(currentOffset);
  } else {
    if (followFeed) {
      ...
      endpoint =
          QString("http://127.0.0.1:8080/api/posts/feed?limit=%1&offset=%2")
              .arg(limit)
              .arg(currentOffset);
    }
  }
  ...
  QNetworkReply *reply = networkManager->get(request);
  connect(reply, &QNetworkReply::finished,
          [this, reply]() { onLoadPostsFinished(reply); });
}
...
void FeedWindow::onCreatePost() {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    QNetworkReply *reply = networkManager->post(request, data);
    connect(reply, &QNetworkReply::finished,
            [this, reply]() { onPostReplyFinished(reply); });
  }
}
...
void FeedWindow::onLikeDislike(const QString &postId, bool isLike) {
  QString endpoint =
      isLike
          ? QString("http://127.0.0.1:8080/api/posts/%1/like").arg(postId)
...
void FeedWindow::updatePostReaction(const QString &postId, int newLikes,
                                    int newDislikes) {
