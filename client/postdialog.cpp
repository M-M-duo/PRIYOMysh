...
PostDialog::PostDialog(QWidget *parent) : QDialog(parent) { setupUI(); }
...
QString PostDialog::cropAndToBase64(const QString &filePath) {
  ...
  if (cropped.width() > maxSize || cropped.height() > maxSize) {
    cropped = cropped.scaled(maxSize, maxSize, Qt::KeepAspectRatio,
...
QStringList PostDialog::getImagesBase64() const { return imagesBase64; }
