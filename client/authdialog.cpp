...
static QString formatPhoneNumber(const QString &raw) {
    ...
    if (digits.length() >= 1) result += digits.mid(1,10);
    return result;
}
...
void AuthDialog::setupUI() {
    ...
    passwordEdit->setStyleSheet("background-color: rgba(200,200,200,0.1); border: none; border-radius: 10px; padding: 8px;");
    ...
    if (okButton) {
        ...
        okButton->setStyleSheet("QPushButton { background-color: rgba(200,200,200,0.6); border: none; border-radius: 10px; font-size: 14px; }"
                                "QPushButton:hover { background-color: rgba(180,180,180,0.8); }");
    }
    if (cancelButton) {
        cancelButton->setFixedSize(66, 33);
        cancelButton->setStyleSheet("QPushButton { background-color: rgba(200,200,200,0.6); border: none; border-radius: 10px; font-size: 14px; }"
                                    "QPushButton:hover { background-color: rgba(180,180,180,0.8); }");
    }
...
QString AuthDialog::getEmail() const { return emailEdit ? emailEdit->text() : QString(); }
QString AuthDialog::getPhone() const { return phoneEdit ? phoneEdit->text() : QString(); }
bool AuthDialog::isPrivate() const { return privateCheckBox ? privateCheckBox->isChecked() : false; }
QString AuthDialog::getAvatarBase64() const { return avatarBase64; }

void AuthDialog::clearField(const QString &fieldName) {
    if (fieldName == "login") loginEdit->clear();
    else if (fieldName == "password") passwordEdit->clear();
    else if (fieldName == "email" && emailEdit) emailEdit->clear();
    else if (fieldName == "phone" && phoneEdit) phoneEdit->clear();
}